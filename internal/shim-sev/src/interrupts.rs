...
use crate::{eprintln, _ENARX_CPUID};
use spinning::Lazy;
...
extern "sysv64" fn vmm_communication_exception_handler(
    ...
    match error_code {
        0x72 => {
            ...
            let (eax, ebx, ecx, edx) =
                match cpuid.get_functions().iter().find(|e| {
                    e.eax_in == stack_frame.rax as u32 && e.ecx_in == stack_frame.rcx as u32
                }) {
                    None => (0, 0, 0, 0),
                    Some(ent) => (ent.eax, ent.ebx, ent.ecx, ent.edx),
                };

            stack_frame.rax = eax as u64;
            stack_frame.rbx = ebx as u64;
            stack_frame.rcx = ecx as u64;
            stack_frame.rdx = edx as u64;
